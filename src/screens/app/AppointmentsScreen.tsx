...
import {
    ...
    FlatList, ScrollView, Linking, ActivityIndicator,
} from "react-native";
...
import {
    User, Settings, ShieldCheck, LogOut, Phone, ChevronRight,
    Info, Car, MessageCircle, Plus, Search, MessageCircleWarning, AlertCircle,
} from "lucide-react-native";

import { useAppointments } from "@/hooks/useAppointments";
import { Appointment } from "@/services/appointmentService";
import { formatPhone, formatPlate } from "@/utils/formatters";

const STATUS_LABELS = ["Todos", "AGENDADO", "CONCLUIDO", "CANCELADO"];
...
const statusConfig: Record<string, { color: string; bg: string }> = {
    AGENDADO: { color: "#F59E0B", bg: "#FEF3C7" },
    CONCLUIDO: { color: "#10B981", bg: "#D1FAE5" },
...
const sendWhatsAppMessage = async (phone: string, clientName: string) => {
    ...
    try {
        ...
        await Linking.openURL(supported ? urlProtocol : urlWeb);
    } catch (error) {
            ...
            {STATUS_LABELS.map((status) => {
                ...
                return (
                    <TouchableOpacity
                        ...
                        style={[styles.button, isSelected ? styles.buttonActive : styles.buttonInactive]}
                    >
...
const renderAppointment = ({ item, navigation }: { item: Appointment; navigation: any }) => {
    const current = statusConfig[item.status] ?? statusConfig["AGENDADO"];

    return (
        <TouchableOpacity
            style={[styles.appointmentCard, { borderLeftColor: current.color }]}
            activeOpacity={0.8}
        ...
        >
            <View style={styles.appointmentInfo}>
                ...
                <View style={styles.infoRow}>
                    ...
                    <Text style={styles.appointmentSubText}>{formatPhone(item.clientPhone)}</Text>
                </View>
                <View style={styles.infoRow}>
                    <Car size={13} color="#94A3B8" />
                    <Text style={styles.appointmentSubText}>
                        {item.vehicleBrand} {item.vehicleModel} • <Text style={styles.plateText}>{formatPlate(item.vehiclePlate)}</Text>
                    </Text>
                </View>
                <View style={[styles.statusBadge, { backgroundColor: current.bg }]}>
                    <Info size={12} color={current.color} />
                    <Text style={[styles.statusText, { color: current.color }]}>{STATUS_DISPLAY[item.status]}</Text>
                </View>
            </View>
            <View style={styles.cardActions}>
                <TouchableOpacity
                    style={styles.whatsappButton}
                    onPress={() => sendWhatsAppMessage(item.clientPhone, item.clientName)}
                >
...
export default function AppointmentsScreen() {
    ...
    const { appointments, loading, error, search, setSearch, status, setStatus } = useAppointments();
...
const styles = StyleSheet.create({
    ...
    button: { paddingHorizontal: 16, paddingVertical: 10, borderWidth: 1, borderRadius: 12 },
    buttonActive: { backgroundColor: "#FFCC00", borderColor: "#FFCC00" },
    buttonInactive: { backgroundColor: "#FFF", borderColor: "#E2E8F0" },
    ...
    appointmentClientName: { fontSize: 16, fontWeight: "700", color: "#1E293B", marginBottom: 6 },
    infoRow: { flexDirection: "row", alignItems: "center", marginBottom: 4 },
    appointmentSubText: { fontSize: 13, color: "#64748B", marginLeft: 6 },
    plateText: { fontWeight: "700", color: "#475569" },
    cardActions: { flexDirection: "row", alignItems: "center", paddingLeft: 10 },
    whatsappButton: {
    ...
    statusBadge: {
        ...
        paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginTop: 12,
    },
